...
# 默认模型数组，便于后续集中调整
DEFAULT_MODELS: List[ELLMType] = [ELLMType.DOUBAO_16_THINKING]
...
def persist_strategy_result(strategy_name: str, llm_type: ELLMType, result: Dict[str, Any]) -> Optional[int]:
...
def run_batch_backtests(strategy_name: str, llm_type: ELLMType,
    ...
    def worker(job: Tuple[str, str]) -> None:
        ...
        if res and 'error' not in res:
            persist_strategy_result(strategy_name, llm_type, res)
        else:
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    for llm_type, raw_code in llm_results:
        try:
            ...
            run_batch_backtests(name_no_ext, llm_type, currencies, intervals, max_workers)
        except Exception as e:
